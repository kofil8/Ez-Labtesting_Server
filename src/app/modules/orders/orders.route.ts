...
router.post('/', auth(Role.CUSTOMER), (req, res, next) => orderController.createOrder(req, res, next));
...
router.post('/:orderId/confirm-payment', auth(Role.CUSTOMER, Role.ADMIN, Role.SUPER_ADMIN), (req, res) =>
  orderController.confirmPayment(req, res),
);
router.post('/:orderId/confirm-order', auth(Role.CUSTOMER, Role.ADMIN, Role.SUPER_ADMIN), (req, res) =>
